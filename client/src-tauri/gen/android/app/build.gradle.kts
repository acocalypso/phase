...
if (releaseBuildRequested) {
    if (!releaseKeystoreFile.isFile) {
        throw GradleException("Release builds require gen/android/keystore.properties")
    }
    releaseKeystoreFile.inputStream().use { releaseKeystoreProperties.load(it) }
    listOf("storeFile", "keyAlias", "password").forEach { key ->
        if (releaseKeystoreProperties.getProperty(key).isNullOrBlank()) {
            throw GradleException("keystore.properties is missing required property: $key")
        }
    }
}
